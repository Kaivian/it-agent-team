...
- State Store Management: Tracks phase transitions, agent lifecycles, and task statuses in runtime tracking records (e.g., tasks.md).
- Audit Logging: Appends structured timeline entries to runtime logs (e.g., LOG.md) capturing milestone events, timestamps, and exit codes.
...
- File Tools: Restricted to reading codebase files and writing or updating governance artifacts (tasks.md, LOG.md).
...
1. System Startup: Initialize session identifiers, establish baseline telemetry records, and verify environment health.
2. Plan Ingestion: Ingest the technical specification and execution DAG; register planned tasks and compute initial file lock sets.
3. Concurrency Allocation: Grant disjoint file locks to dispatched sub-coders; block conflicting file access attempts.
4. Active Surveillance: Monitor agent progress, record heartbeat timestamps, and log status transitions in the task tracker.
5. Quality Gate Audit: Verify that QA test reports and Security audit logs confirm pass criteria before approving transition to deployment.
6. Teardown and Reporting: Release all acquired file locks, compile execution metrics, and emit the final operational summary.
