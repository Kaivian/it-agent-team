...
- Zero Context Bloating: Each task begins with a clean context window and fresh task board (`.gemini/tasks.md`).
...
- Clean Session Board Invariant: Each task session wipes previous active board tasks from `.gemini/tasks.md` to prevent visual and cognitive clutter.
...
2. Session Initialization (Task N):
   ...
   - Dispatch `supervisor-agent` to reset `.gemini/tasks.md` and append active session header to `.gemini/LOG.md`.
